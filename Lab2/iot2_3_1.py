...
def readTemp(bluetooth):
    try:
        while True:
            bluetooth.expect('Notification handle = 0x002a value: 34.*')
            rval = bluetooth.after.split()
...
def createKinesisData():
    ...
    res += "measurement times: " + str(num) + " current temperature: " + str(tmp) + " local time: " + localtime
    return res
...
try:
        while (1):
            ...
            if (chooseDB):
                ...
                myLcd.write(lcdPrint)
                #db code
                print time.strftime('%Y-%m-%d',time.strftime("%Y-%m-%d %A %X %Z", time.localtime())  )
                users.put_item(data={'iteration'=measureNum, 'temp'=curTemp, 'time'=time.strftime("%Y-%m-%d %A %X %Z", time.localtime())})
            else:
